...
class Omnimatte(nn.Module):
    ...
    def forward(self, input, bg_flow, bg_warp, jitter, index, do_adj):
        ...
        bg_warp = torch.cat((bg_warp[:, :2], bg_warp[:, 2:])) #cat没指定维度，默认为0维，(16,4,256,448) --> (32,2,256,448)
        ...
        bg_offset = F.interpolate(self.bg_offset, (self.max_frames, 4, 7), mode='trilinear', align_corners=True)#(1,2,20,4,7) --> (1,2,200,4,7)
        #插值，5d（batch,channel,depth,width,height）,包含时间序列
        bg_offset = bg_offset[0, :, index].transpose(0, 1) #(1,2,200,4,7) --> (32,2,4,7)，从200个里取32个

        bg_offset = F.grid_sample(bg_offset, jitter.permute(0, 2, 3, 1), align_corners=True) #(32,2,4,7) --> (32,2,256,448)，将其恢复成整幅图的形状
        #print("0,",np.all(bg_offset.cpu().detach().numpy()==0))
        #print("1,", np.all(self.brightness_scale.cpu().detach().numpy() == 1))
        ...
        for i in range(n_layers):
            ...
            if i == 0:
                ...
                rgba = F.grid_sample(rgba, bg_warp.permute(0, 2, 3, 1), align_corners=True) #相机位置变了，背景也要动
        ...
        #print("layers_rgba1=",np.array(layers_rgba).size())
