...
class OmnimatteDataset(BaseDataset):
    ...
    def get_transformed_item(self, index, transform_params):
        ...
        #print("input_flow_size=",input_flow.size())
    ...
    def get_background_uv(self, index, w, h): #h=256,w=448
        """Return background layer UVs at 'index' (output range [0, 1])."""
        #print("h=",h)
        #print("w=",w)
        ramp_u = torch.linspace(0, 1, steps=w).unsqueeze(0).repeat(h, 1)  # 448 --> (1,448) --> (256.448) repreat:在第一维度上复制h次，第二个维度复制1次
        #print("ramp_u=",ramp_u.size())
        ramp_v = torch.linspace(0, 1, steps=h).unsqueeze(-1).repeat(1, w) # 256 -->(256,1) -->(256,448) repreat：在第一维度复制1次（不变），第二维度复制w次
        #print("ramp_v=",ramp_v.size())
        ramp = torch.stack([ramp_u, ramp_v], 0)
        #print("ramp_size111111111=",ramp.size())
        if hasattr(self, 'homographies'):
            # scale to [0, orig width/height]
            ramp[0] *= self.h_scale_x
            ramp[1] *= self.h_scale_y
            # apply homography
            ramp = ramp.reshape(2, -1)  # [2, H, W] --> [2,HxW]
            #print(ramp[0])
