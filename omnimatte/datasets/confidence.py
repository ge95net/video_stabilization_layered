...
def compute_confidence(flo_f, flo_b, rgb, thresh=1, thresh_p=20):
    ...
    warped_2 = F.grid_sample(warped_1, torch.from_numpy(warp_f), align_corners=True).numpy()#先正向求，再逆向返回来求，比较差异是否大
    err = np.linalg.norm(warped_2 - identity_grid, axis=1)
    err[err > thresh] = thresh  #err若大于阈值，
    err /= thresh
    confidence = 1 - err
...
def main(dataroot):
    ...
    forward_flo = sorted(glob.glob(os.path.join(dataroot, 'flow', '*.flo')))
    backward_flo = sorted(glob.glob(os.path.join(dataroot, 'flow_backward', '*.flo')))
    assert(len(forward_flo) == len(backward_flo))
    rgb_paths = sorted(glob.glob(os.path.join(dataroot, 'rgb', '*')))
    print(f'generating {len(forward_flo)} confidence maps...')
    outdir = os.path.join(dataroot, 'confidence')
